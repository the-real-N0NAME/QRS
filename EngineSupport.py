...
def aligne_sections(sections):
    """
    Aligns sections for the user to select
    """
    aligned_sections = []
    for i in sorted(sections):
        aligned_sections.append(f"{i} -> {sections[i]}\n")
    return aligned_sections


def ask_section(sections: ConfigParser, OverwriteInput = None):
    """
    Prompt the user which profile should be used for decryption
    """
    # Do not ask for choice if user already gave one
    choice = "ASK"
    if OverwriteInput is not None:
        choice = OverwriteInput
    #while choice not in sections and OverwriteInput == None:
    #    print("importing dependencies...\n")
    #    from client import AskServer
    #    sys.stderr.write("Select the Mozilla profile you wish to decrypt\n")
    #    print_sections(sections)
    #    AskServer(f"Select the Mozilla profile you wish to decrypt\n {sections}")
    #    raise Exit(Exit.ERROR)

    try:
        final_choice = sections[choice]
        print(f"Selected profile: {final_choice}")
    except KeyError:
...
def get_profile(
    basepath: str, interactive: bool, choice: Optional[str], list_profiles: bool, OverwriteInput = None
):
    """
    ...
    try:
        ...
        if len(sections) == 1:
            ...
            if OverwriteInput != None:
                section = ask_section(sections, OverwriteInput)
            else:
...
def main(OverwriteInput = None) -> None:
    """Main entry point"""
    args = parse_sys_args()
    ...
    if OverwriteInput != None and OverwriteInput != "GetProfiles":
        profile = get_profile(basepath, args.interactive, args.choice, args.list, OverwriteInput)
    elif OverwriteInput == "GetProfiles":
        return get_profile(basepath, args.interactive, args.choice, args.list, OverwriteInput)
    else:
...
def write_to_file(output):
    with open('ExtractedPasswords.txt', 'w') as file:
        file.write(output)
